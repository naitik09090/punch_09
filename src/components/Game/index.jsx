...
const Game = () => {
  ...
  const handleTimerComplete = () => {
    // Stage cleared! Add 1 minute bonus and increase speed
    if (!muted) playSparkle() // Sound effect or subtle feedback

    // Add 1 minute
    if (timerRef.current) {
      timerRef.current.addTime(60000)
    }

    // Make moles faster (decrease speed/duration by 10%)
    setMoles((currentMoles) =>
      currentMoles.map((mole) => ({
        ...mole,
        speed: Math.max(0.1, mole.speed * 0.9), // Cap at 0.1s
        delay: Math.max(0.1, mole.delay * 0.9), // Cap at 0.1s
      }))
    )
  }
      ...
      {playing && (
        <Fragment>
          ...
          <div className="game-info">
            <Score value={score} />
            <Timer ref={timerRef} time={constants.TIME_LIMIT} onEnd={handleTimerComplete} />
          </div>
        </Fragment>
      )}
