...
const Mole = ({
  ...
  const setMole = useCallback(
    (
      ...
      // 10% Chance for BOMB (Danger!)
      const isBomb = rand < 0.1 && !override;
      // 10% Chance for HAMSTER (Friendly! Don't hit!)
      const isHamster = rand >= 0.1 && rand < 0.2 && !override;
      // Rest: Regular Moles (or Golden)

      if (isBomb) {
        // Create BOMB
        pointsRef.current = -50;
        typeRef.current = 'bomb';
        gsap.set([capRef.current, specsRef.current], { display: 'none' });
        gsap.set(fuseRef.current, { display: 'block' });
        gsap.set(moleContainerRef.current, {
          ...
          '--hue': 0,
          '--lightness': 10,
          '--body-fill': '#2d3436', // Black Bomb
          '--feature-color': '#ff0000', // Red eyes/spark
        })
      } else if (isHamster) {
        // Create HAMSTER
        pointsRef.current = -20;
        typeRef.current = 'hamster';
        gsap.set([capRef.current, specsRef.current, fuseRef.current], { display: 'none' });
        gsap.set(moleContainerRef.current, {
          '--accent': 280,
          '--shades': 0,
          '--golden': 0,
          '--hue': 30,
          '--lightness': 90,
          '--body-fill': '#ecf0f1', // White/Light Hamster
          '--feature-color': '#00b894', // Green/Safe look
        })
      } else if (rand > 0.98 || override) {
        // Create the "Golden" Mole
        pointsRef.current = constants.GOLDEN_SCORE
        typeRef.current = 'golden';
        gsap.set([capRef.current, specsRef.current], { display: 'block' });
        gsap.set(fuseRef.current, { display: 'none' });
        gsap.set(moleContainerRef.current, {
          '--accent': accent,
          '--shades': shades,
          '--golden': golden,
        ...
        gsap.set(moleContainerRef.current, {
          ...
          '--body-fill': '#636e72', // Default Grey/Dark body
          '--feature-color': '#ffffff',
        })
      }
    },
  ...
  useEffect(() => {
    ...
    if (active) {
      ...
      bobRef.current = gsap.to([buttonRef.current, moleRef.current], {
        ...
        yoyo: true,
        repeat: -1,
        // Make bombs and hamsters move faster to tricky you
        timeScale: pointsRef.current < 0 ? 1.2 : 1,
        repeatDelay: delay,
        delay,
        onRepeat: () => {
          // If it was a special type (neg points), reset logic to random again
          if (pointsRef.current < 0) setMole();
          else {
  ...
  const renderScore = (x, y, value) => {
    ...
    SCORE.innerText = value > 0 ? `+${value}` : value
    if (value < 0) SCORE.style.color = '#ff4d4d' // Red for negative
  ...
  const whack = (e) => {
    e.stopPropagation() // Prevent triggering the miss handler on the hole
    setWhacked(true)
    renderScore(e.pageX, e.pageY, pointsRef.current)
    onWhack(pointsRef.current, typeRef.current)
  }

  const miss = (e) => {
    // We strictly want to penalize hitting an EMPTY hole.
    // We verify the game is active and the specific mole isn't 'whacked'
    if (active && !whacked && !loading) {
      // Check vertical position (0 = Up/Visible, 100 = Down/Hidden)
      const yPercent = gsap.getProperty(moleRef.current, "yPercent");

      // Safety check: ensure we have a number
      const pos = typeof yPercent === 'number' ? yPercent : 100;

      // If the mole is mostly UP (visible), we DO NOT count this as a miss.
      // This prevents "unfair" misses when you click the hole edge but the mole is there.
      if (pos < 60) return;

      const penalty = 5
      renderScore(e.pageX, e.pageY, -penalty)
      if (onMiss) onMiss(penalty)
    }
  }
  ...
  return (
    <div className="mole__hole" ref={moleContainerRef} onPointerDown={miss}>
      <svg
        className="mole"
        viewBox="0 0 200 200"
        fill="none"
        ...
        <g clipPath="url(#mole-clip)">
          <g className="mole__mole" ref={moleRef}>
            ...
            <g className="mole__fuse" ref={fuseRef} style={{ display: 'none' }}>
              <path
                d="M100 20 Q 110 5, 125 10"
                fill="none"
                stroke="#7f8c8d"
                strokeWidth="4"
              />
              <circle cx="125" cy="10" r="4" fill="#e74c3c">
                <animate attributeName="opacity" values="1;0.2;1" dur="0.2s" repeatCount="indefinite" />
              </circle>
              <circle cx="125" cy="10" r="6" fill="yellow" opacity="0.5">
                <animate attributeName="r" values="4;8;4" dur="0.4s" repeatCount="indefinite" />
              </circle>
            </g>
          </g>
        </g>
